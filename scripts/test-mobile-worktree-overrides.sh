...
grep -q '^BUNDLE_IDENTIFIER = com\.buzz\.buzzMobile$' "$debug_xcconfig" \
  && pass "Debug.xcconfig defaults to the fork bundle identifier" \
  || fail "Debug.xcconfig must default to com.buzz.buzzMobile"
...
grep -q '^ios_prefix="com.buzz.buzzMobile\."$' "$clean_script" \
  && pass "cleanup targets the iOS fork worktree prefix" \
  || fail "cleanup must share the iOS fork prefix used by worktree overrides"
