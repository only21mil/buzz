...
def prepare(
    ...
    write_private(state_dir / "capability-parity-receipt.json", canonical_json(receipt))
    claim = os.urandom(32)
    ...
    state = {
        ...
        "state": "prepared",
        "binding": binding,
        "sealed_receipt_sha256": receipt["sealed_sha256"],
...
def complete_phase(state_dir: Path, slug: str, gate_path: Path) -> dict[str, Any]:
    ...
    expected_fields = {"schema", "status", "slug", "binding", "gates", "channel_set_sha256"}
    ...
    manifest = load_json(state_dir / "bundle-manifest.json")
    expected_channels = manifest.get("capability_parity", {}).get("eligible_channels_sha256")
    if gate.get("channel_set_sha256") != expected_channels:
        raise TransactionError("phase channel set is not manifest-bound")
