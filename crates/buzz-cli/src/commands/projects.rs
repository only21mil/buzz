...
use buzz_sdk::{build_project, build_project_with_tags, ProjectMemberCoord, PROJECT_D_MAX_LEN};
...
pub async fn cmd_delete(client: &BuzzClient, slug: &str) -> Result<(), CliError> {
    ...
    // Exact-event deletion cannot erase a replacement signed during this request.
    let tombstone = EventBuilder::new(nostr::Kind::Custom(5), "")
        .tags([Tag::event(head.id)])
        .custom_created_at(next_ts);
...
mod tests {
    ...
    fn next_timestamp_returns_head_plus_one_when_head_is_ahead_of_wall_clock() {
        ...
        let next = next_timestamp(&head, Timestamp::now()).expect("no overflow");
        assert_eq!(
            next.as_secs(),
