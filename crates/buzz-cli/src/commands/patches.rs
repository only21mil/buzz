...
pub async fn cmd_list_patches(
    ...
    let mut filter = serde_json::json!({"kinds": [1617]});

    if let Some(pk) = author {
        validate_hex64(pk)?;
        filter["authors"] = serde_json::json!([pk]);
    }
    let events = query_repo_events(client, filter, &a_value, None, limit).await?;
    let resp = serde_json::to_string(&events)
        .map_err(|e| CliError::Other(format!("failed to serialize patch list: {e}")))?;
    println!("{resp}");
    Ok(())
}
