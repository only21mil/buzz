mod external_infra_tests {
    ...
    #[tokio::test]
    #[ignore = "requires disposable migrated Postgres fixture and git storage"]
    async fn community_admin_commands_preserve_tenant_owner_delegation_and_audit_guards() {
        ...
        let wrong_kind =
            buzz_sdk::nip_oa::compute_auth_tag(&owner, &delegate.public_key(), "kind=9002")
                .unwrap();
        assert!(submit(
            &state,
            &tenant,
        ...
        .await
        .is_err());
        let grant = buzz_sdk::nip_oa::compute_auth_tag(&owner, &delegate.public_key(), "kind=9001")
            .unwrap();
        submit(
            &state,
            &tenant,
    ...
    #[tokio::test]
    #[ignore = "requires disposable Postgres fixture and git storage; run against migrations and desired schema"]
    ...
    #[tokio::test]
    #[ignore = "requires disposable Postgres fixture and git storage"]
