...
fn set_close_on_exec(fd: &impl AsFd) -> std::io::Result<()> {
...
mod tests {
    ...
    const DAC_PASS_MARKER: &str = "buzz-execd-dac:pass-v1";
    const DAC_SKIP_MARKER: &str = "buzz-execd-dac:skip-no-usable-mapped-nonroot-principal-v1";
    ...
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    enum DacPrincipalProbe {
        Usable,
        Unusable,
    }

    fn classify_dac_principal_probe(result: Result<(), Errno>) -> Result<DacPrincipalProbe, Errno> {
        match result {
            Ok(()) => Ok(DacPrincipalProbe::Usable),
            Err(Errno::EPERM) => Ok(DacPrincipalProbe::Unusable),
            Err(error) => Err(error),
        }
    }

    fn mapped_nonroot_id(map: &str) -> io::Result<Option<u32>> {
        let invalid = |line: usize, reason: &str| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid id map line {line}: {reason}"),
            )
        };
        let limit = u64::from(u32::MAX) + 1;
        let mut selected = None;

        for (index, raw_line) in map.lines().enumerate() {
            ...
            if fields.len() != 3 {
                return Err(invalid(line_number, "expected exactly three fields"));
            }
            let parse = |field: &str| {
                field
                    .parse::<u64>()
                    .map_err(|_| invalid(line_number, "field is not an unsigned decimal integer"))
            };
            let inner = parse(fields[0])?;
            let outer = parse(fields[1])?;
            let count = parse(fields[2])?;
            if count == 0 {
                return Err(invalid(line_number, "range count is zero"));
            }
            let inner_end = inner
                .checked_add(count)
                .ok_or_else(|| invalid(line_number, "inner range overflows"))?;
            let outer_end = outer
                .checked_add(count)
                .ok_or_else(|| invalid(line_number, "outer range overflows"))?;
            if inner_end > limit || outer_end > limit {
                return Err(invalid(line_number, "range exceeds the u32 id space"));
            }

            let candidate = inner.max(1);
            if selected.is_none() && candidate < inner_end && candidate != u64::from(u32::MAX) {
    ...
    fn mapped_nonroot_id_from(path: &Path) -> io::Result<Option<u32>> {
        mapped_nonroot_id(&fs::read_to_string(path)?)
    }

    fn dac_coverage_marker(output: &[u8]) -> io::Result<DacCoverage> {
        let mut markers = output.split(|byte| *byte == b'\n').filter_map(|line| {
            if line == DAC_PASS_MARKER.as_bytes() {
                Some(DacCoverage::Pass)
            } else if line == DAC_SKIP_MARKER.as_bytes() {
                Some(DacCoverage::Skip)
            } else {
                None
            }
        });
        let marker = markers.next().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidData, "DAC child emitted no marker")
        })?;
        if markers.next().is_some() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "DAC child emitted multiple markers",
            ));
        }
        Ok(marker)
    }
    ...
    #[test]
    fn executor_rpc_rejects_caller_controlled_process_fields() {
        let request = executor_request_fixture(
            "executor_handoff",
            [41; 32],
            [7; 16],
        ...
        for field in ["argv", "env", "program", "cwd", "path"] {
            ...
            value
                .as_object_mut()
                .unwrap()
                .insert(field.into(), serde_json::json!("caller-controlled"));
            let bytes = serde_json::to_vec(&value).unwrap();
            assert!(serde_json::from_slice::<ExecutorRequest>(&bytes).is_err());
            assert!(canonical_parse::<ExecutorRequest>(&bytes).is_err());
        }
    }

    #[test]
    fn accepted_executor_stream_is_closed_across_fixture_exec() {
        let (stream, _peer) = UnixStream::pair().unwrap();
        fcntl(&stream, FcntlArg::F_SETFD(FdFlag::empty())).unwrap();
        set_close_on_exec(&stream).unwrap();
        let flags = fcntl(&stream, FcntlArg::F_GETFD).unwrap();
    ...
    fn executor_partial_start_and_restart_recovery_are_fail_closed() {
        ...
        let mut second_handoff = request("executor_handoff");
        second_handoff.execution_binding_digest = hex::encode([43; 32]);
        second_handoff.attempt_id = hex::encode([9; 16]);
        assert!(
            executor_transition(second_handoff, &mut active, Path::new("/nonexistent")).is_err()
        );
        assert_eq!(active.len(), 1);
        assert!(active.contains_key(&retained_binding));
    ...
    fn fixed_executor_runs_frozen_fixture_and_returns_exact_bounded_outputs() {
        ...
        let materialized_input = attempts
            .join(hex::encode(attempt_id))
            .join(MATERIALIZED_SOURCE_ROOT)
            .join(FIXTURE_TREE.join("/"))
            .join(FIXTURE_INPUT_NAME);
        fs::set_permissions(&materialized_input, fs::Permissions::from_mode(0o600)).unwrap();
        fs::write(&materialized_input, b"tampered after execution\n").unwrap();
        fs::set_permissions(&materialized_input, fs::Permissions::from_mode(0o400)).unwrap();
        assert!(system.sealed_artifacts(binding).is_err());
        fs::set_permissions(&materialized_input, fs::Permissions::from_mode(0o600)).unwrap();
        fs::write(
            &materialized_input,
            include_bytes!("../../../deploy/native-ci/acceptance/fixtures/input.txt"),
    ...
    #[test]
    fn mapped_nonroot_id_strictly_validates_and_selects_the_first_eligible_id() {
        assert_eq!(mapped_nonroot_id("").unwrap(), None);
        assert_eq!(mapped_nonroot_id(" \n\t\n").unwrap(), None);
        assert_eq!(mapped_nonroot_id("0 1000 1\n").unwrap(), None);
        assert_eq!(mapped_nonroot_id("0 1000 2\n").unwrap(), Some(1));
        assert_eq!(mapped_nonroot_id("0 1000 1\n5 2000 3\n").unwrap(), Some(5));
        assert_eq!(
            mapped_nonroot_id("0 1001 1\n1 100000 65535\n").unwrap(),
            Some(1)
        );
        assert_eq!(
            mapped_nonroot_id("0 1001 1\n1000 200000 1\n").unwrap(),
        ...
        assert_eq!(mapped_nonroot_id("0 1001 1\n").unwrap(), None);
        assert_eq!(
            mapped_nonroot_id("4294967294 0 1\n").unwrap(),
            Some(u32::MAX - 1)
        );
        assert_eq!(mapped_nonroot_id("4294967295 0 1\n").unwrap(), None);
        assert_eq!(
            mapped_nonroot_id("0 1001 1\n4294967295 200000 1\n").unwrap(),
            None
        );
        ...
        for invalid in [
            ...
            assert!(
                mapped_nonroot_id(invalid).is_err(),
                "accepted invalid map {invalid:?}"
            );
        }
    }

    #[test]
    fn dac_coverage_marker_requires_exactly_one_exact_marker() {
        assert_eq!(
            dac_coverage_marker(format!("noise\n{DAC_PASS_MARKER}\n").as_bytes()).unwrap(),
            DacCoverage::Pass
        );
        assert_eq!(
            dac_coverage_marker(format!("{DAC_SKIP_MARKER}\nnoise\n").as_bytes()).unwrap(),
            DacCoverage::Skip
        );
        for invalid in [
            Vec::new(),
            format!("{DAC_PASS_MARKER}-suffix\n").into_bytes(),
            format!("{DAC_PASS_MARKER}\n{DAC_SKIP_MARKER}\n").into_bytes(),
            format!("{DAC_PASS_MARKER}\n{DAC_PASS_MARKER}\n").into_bytes(),
        ] {
            assert!(dac_coverage_marker(&invalid).is_err());
        }
    }

    #[test]
    fn dac_principal_probe_skips_only_the_confirmed_host_capability_error() {
        assert_eq!(
            classify_dac_principal_probe(Ok(())).unwrap(),
            DacPrincipalProbe::Usable
        );
        assert_eq!(
            classify_dac_principal_probe(Err(Errno::EPERM)).unwrap(),
            DacPrincipalProbe::Unusable
        );
        for unexpected in [Errno::EINVAL, Errno::EOPNOTSUPP, Errno::EIO] {
            assert_eq!(
                classify_dac_principal_probe(Err(unexpected)).unwrap_err(),
                unexpected
            );
        }
    }
    ...
    fn privilege_dropped_job_traverses_only_frozen_attempt_and_seccomp_paths() {
        ...
        const REQUIRE_PASS: &str = "BUZZ_EXECD_DAC_REQUIRE_PASS";
        ...
        if std::env::var_os(ROOT_HELPER).is_some() {
            let uid = mapped_nonroot_id_from(Path::new("/proc/self/uid_map")).unwrap();
            let gid = mapped_nonroot_id_from(Path::new("/proc/self/gid_map")).unwrap();
            let (Some(uid), Some(gid)) = (uid, gid) else {
                eprintln!("{DAC_SKIP_MARKER}");
                return;
            };
            let temporary = tempfile::Builder::new()
                ...
                .tempdir_in("/tmp")
                .unwrap();
            let principal_probe = temporary.path().join("mapped-principal-probe");
            fs::write(&principal_probe, b"probe\n").unwrap();
            match classify_dac_principal_probe(nix::unistd::chown(
                &principal_probe,
            ...
            .unwrap()
            {
                DacPrincipalProbe::Usable => fs::remove_file(principal_probe).unwrap(),
                DacPrincipalProbe::Unusable => {
                    eprintln!("{DAC_SKIP_MARKER}");
                    return;
                }
            ...
            fn assign_job(path: &Path, uid: Uid, gid: Gid) {
                if path.is_dir() {
                    for entry in fs::read_dir(path).unwrap() {
                        assign_job(&entry.unwrap().path(), uid, gid);
                    }
                }
                nix::unistd::chown(path, Some(uid), Some(gid)).unwrap();
            }
            assign_job(&attempt, Uid::from_raw(uid), Gid::from_raw(gid));
        ...
        let coverage = dac_coverage_marker(&output.stderr).unwrap();
        if std::env::var_os(REQUIRE_PASS).is_some() {
            assert_eq!(
                coverage,
                DacCoverage::Pass,
                "qualified-host DAC coverage was required but the child skipped"
            );
        }
    }
