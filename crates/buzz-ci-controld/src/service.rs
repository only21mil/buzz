...
impl CapacityZeroService {
    ...
    pub(crate) fn acceptance_credentials(&self) -> Option<(u32, u32, Duration)> {
        self.acceptance
            .as_ref()
            .map(|journal| (journal.peer_uid(), journal.peer_gid(), journal.timeout()))
    }
...
impl CapacityOneService {
    ...
    pub(crate) fn acceptance_credentials(&self) -> (u32, u32, Duration) {
        (
            self.acceptance.peer_uid(),
            self.acceptance.peer_gid(),
...
mod tests {
    ...
    fn reports_ready_but_closed_after_store_validation() {
        ...
        let service = CapacityZeroService::start(&config, owner_uid, None).expect("service starts");
