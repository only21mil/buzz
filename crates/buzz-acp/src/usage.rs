...
impl StandardUsageTracker {
    ...
    pub(crate) fn begin_turn(&mut self, session_id: &str) {
        for closed in self.close_after_take.drain() {
    ...
    /// Forget a deleted session after preserving any final usage for the drain.
    pub(crate) fn close_session(&mut self, session_id: &str) {
        ...
        {
            self.close_after_take.insert(session_id.to_owned());
        } else {
    ...
    pub(crate) fn take(&mut self) -> Option<TurnUsage> {
        ...
        if inclusive_input.is_none() && cumulative_cost.is_none() {
            if self.close_after_take.remove(&session_id) {
                self.sessions.remove(&session_id);
            }
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn standard_close_drains_final_metric_and_resume_keeps_baseline() {
        let mut tracker = StandardUsageTracker::default();
        tracker.seed_zero_baseline("session");
        tracker.begin_turn("session");
        tracker.record_cost("session", 1.0);
        assert_eq!(tracker.take().unwrap().turn_cost_usd, Some(1.0));
        // Resume does not seed or delete; the previous cost remains the baseline.
        tracker.begin_turn("session");
        tracker.record_cost("session", 1.5);
        tracker.close_session("session");
        let last = tracker.take().unwrap();
        assert_eq!(last.turn_cost_usd, Some(0.5));
        assert_eq!(last.turn_seq, 2);
        assert!(!tracker.sessions.contains_key("session"));
