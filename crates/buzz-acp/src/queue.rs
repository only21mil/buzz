...
impl EventQueue {
    ...
    pub fn push_detailed(&mut self, event: QueuedEvent) -> PushOutcome {
        ...
        let evicted = if queue.len() >= MAX_PENDING_PER_SCOPE {
        ...
        let aggregate_evicted = self.enforce_channel_cap(channel_id, false);
        PushOutcome {
            accepted: true,
            evicted: evicted.or(aggregate_evicted),
        }
    }
    ...
    /// Enforce the aggregate channel cap. Admission evicts the oldest head;
    /// recovery evicts the newest tail so original held/retried work survives.
    fn enforce_channel_cap(&mut self, channel_id: Uuid, newest: bool) -> Option<Event> {
        ...
        while self.channel_event_total(channel_id) > MAX_PENDING_PER_CHANNEL {
            ...
            let victim = if newest {
                candidates.max_by_key(|(_, queue)| queue.back().map(|event| event.received_at))
            } else {
                candidates.min_by_key(|(_, queue)| queue.front().map(|event| event.received_at))
            }
            .map(|(scope, _)| scope.clone());
            let Some(scope) = victim else { break };
            if let Some(q) = self.queues.get_mut(&scope) {
                evicted =
                    if newest { q.pop_back() } else { q.pop_front() }.map(|queued| queued.event);
                if q.is_empty() {
            ...
            tracing::warn!(
                ...
                newest,
                "aggregate per-channel queue cap reached — dropped event"
            );
        }
        evicted
    }
    ...
    pub fn requeue(&mut self, batch: FlushBatch) -> Option<FlushBatch> {
        ...
        self.enforce_channel_cap(channel_id, true);
        None
    }
...
mod tests {
    ...
    #[test]
    fn held_thread_requeue_preserves_original_when_siblings_fill_channel_cap() {
        let channel = Uuid::new_v4();
        let scope_a = thread(channel, &"a".repeat(64));
        let scope_b = thread(channel, &"b".repeat(64));
        let mut queue = EventQueue::new(DedupMode::Queue);
        let mut original = make_queued(channel, "held original");
        original.scope = scope_a.clone();
        let original_id = original.event.id;
        queue.push(original);
        let held = queue.flush_next().unwrap();
        for index in 0..MAX_PENDING_PER_CHANNEL {
            let mut queued = make_queued(channel, &format!("sibling {index}"));
            queued.scope = scope_b.clone();
            queue.push(queued);
        }
        queue.requeue_preserve_timestamps(held);
        queue.mark_complete(&scope_a);
        assert_eq!(queue.channel_event_total(channel), MAX_PENDING_PER_CHANNEL);
        let replay = queue.flush_next().unwrap();
        assert_eq!(replay.scope, scope_a);
        assert_eq!(replay.events[0].event.id, original_id);
    }
    ...
    fn test_format_prompt_dm_reply_with_complete_thread_context_omits_retrieval_hint() {
        ...
        let event = make_event_with_tags(
            "sounds good, do it",
            vec![vec!["e".into(), "a".repeat(64), "".into(), "reply".into()]],
        );
        let batch = FlushBatch {
            channel_id: ch,
            scope: conv(ch),
