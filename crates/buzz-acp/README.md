...
| `/plan [text]` | Live | Harness, then connector | When the seat's session advertised a `plan` mode at `session/new` (claude-agent-acp does), the harness sets `session/set_config_option {mode: "plan"}` on the live session, posts "Plan mode on for this turn." in-thread, prompts with `text` (or "Build a plan for the current thread.") as the first block, answers any `ExitPlanMode` permission request during the turn with the connector's keep-planning option (`optionId: "plan"`, a `reject_once`), and restores the configured `--permission-mode` after the turn. A turn ended by a control signal leaves the session in plan mode; the next prompt on that scope restores the mode first, and session invalidation drops the flag. A seat without a `plan` mode that advertises `/plan` (codex-acp) gets the command untouched, so its own toggle answers (codex-acp rejects arguments itself). A seat with neither gets an in-thread reply and no prompt. Claude Code's own `/plan` is never reached: the harness owns the name. A second `/plan` inside a steer-merged batch is plain text, since only a single-event batch qualifies as a slash command. |
...
`/stop` lives in `stop_command.rs`; `/plan` in `pool.rs` (`run_prompt_task`,
`end_plan_turn`) and `acp.rs` (`choose_permission_option`, `set_plan_hold`).
...
DMs are ordinary kind:9 messages in a channel whose kind:39000 metadata carries `["t", "dm"]`, and they carry no `p` tag. In `subscribe=mentions` mode the harness therefore drops the relay-side `#p` filter for every channel the relay identifies as a DM, both at startup discovery and when a member-added notification (kind:44100) arrives for a new DM. If that metadata cannot be fetched at subscription time, the channel is subscribed with the mention filter and the harness retries the lookup every 10 seconds (one bounded lookup per tick, off the event loop, up to 30 attempts). Once the channel resolves as a DM it re-issues the subscription under the same id without `#p`; the REQ `since` is the lower of the channel's last seen event and the original subscription floor, so DMs posted while the channel was mention-gated are replayed even if a later @mention already advanced the channel's cursor. Replayed events already processed are dropped by the inbox cursor. Non-DM channels keep the explicit-mention subscription, and the inbound author gate is unchanged: inside a DM only the owner and verified same-owner siblings fire a turn.
