...
fn signer_and_verifier_are_deterministic_and_fail_closed() {
    ...
    let mut unsigned = json!({
        ...
        "schema": "buzz-agent-capability-parity-receipt-v1",
        "status": "PASS"
    });
    let mut canonical = serde_json::to_vec(&unsigned).unwrap();
    canonical.push(b'\n');
    ...
    let mut persisted_canonical = serde_json::to_vec(&persisted).unwrap();
    persisted_canonical.push(b'\n');
