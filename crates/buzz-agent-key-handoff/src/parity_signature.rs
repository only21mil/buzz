...
pub fn verify_envelope(input: &[u8], expected_owner: &str) -> Result<()> {
    ...
    match (envelope.verified, envelope.sealed_sha256.as_deref()) {
        ...
        (true, Some(recorded)) if lowercase_hex_64(recorded) => {
            ...
            let mut canonical = serde_json::to_vec(&persisted)
                .context("serialize persisted verification envelope")?;
            canonical.push(b'\n');
    ...
    let mut canonical = serde_json::to_vec(&Value::Object(unsigned_receipt))
        .context("serialize receipt payload")?;
    canonical.push(b'\n');
