...
fn parse_profile(e: &HashMap<String, String>) -> Result<AppProfileConfig, ConfigError> {
    ...
    // One server-owned application identity binds App Attest and APNs.
    // An identity for another bundle must never enroll under this topic.
    let suffix = format!(".{apns_topic}");
    if !app_attest_app_id.strip_suffix(&suffix).is_some_and(|team| {
        !team.is_empty()
            && team
                .bytes()
                .all(|byte| byte.is_ascii_uppercase() || byte.is_ascii_digit())
    }) {
        return Err(ConfigError::Invalid(app_id_key));
    }
...
mod tests {
    ...
    #[test]
    fn app_attest_identity_must_match_the_configured_apns_topic() {
        for identity in [
            "TEAM.other.bundle",
            "team.xyz.block.buzz.dogfood.mobile",
            ".xyz.block.buzz.dogfood.mobile",
        ] {
            let mut env = base();
            env.insert(
                "BUZZ_PUSH_DOGFOOD_APP_ATTEST_APP_ID".into(),
            ...
            assert!(matches!(
                Config::from_map(&env),
                Err(ConfigError::Invalid("BUZZ_PUSH_DOGFOOD_APP_ATTEST_APP_ID"))
            ));
        }
        let mut env = base();
        env.insert(
            "BUZZ_PUSH_DOGFOOD_APNS_TOPIC".into(),
            "com.buzz.buzzMobile".into(),
        );
        env.insert(
            "BUZZ_PUSH_DOGFOOD_APP_ATTEST_APP_ID".into(),
            "TEAM.com.buzz.buzzMobile".into(),
        );
