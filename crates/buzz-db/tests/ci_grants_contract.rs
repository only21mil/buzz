...
async fn owner_grant_is_durable_and_visible_to_get_active_ci_signers() {
    ...
    assert_eq!(signers.len(), 1, "re-upsert must not duplicate the grant row");
    assert_eq!(signers[0], signer);
}
...
async fn rejected_non_owner_grant_is_never_visible() {
    ...
    let signers = get_active_ci_signers(&fixture.pool, fixture.community, fixture.channel, &fixture.repo_a(), now)
        .await
        .expect("query other repo");
