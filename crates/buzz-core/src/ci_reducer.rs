...
pub enum CiReducedState {
    /// The run has not reached a terminal result.
    Pending,
    /// Every required job completed successfully.
    Green,
    /// At least one required job completed unsuccessfully.
    Red,
    /// The run cannot be reduced to a trustworthy product result.
...
pub struct CiReducedJob {
    ...
    pub job_id: String,
    /// Human-readable job name when supplied by the workflow.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Latest accepted state for the selected attempt.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub state: Option<CiJobState>,
    /// Whether the workflow marks this job as required.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required: Option<bool>,
    /// Runner-reported start time in Unix seconds.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub started_at: Option<u64>,
    /// Runner-reported completion time in Unix seconds.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct CiReduction {
    /// Run identifier copied from the validated request.
    pub run_id: String,
    /// Immutable source object identifier copied from the request.
    pub sha: String,
    /// Selected run attempt.
    pub attempt: u32,
    /// Aggregate trusted state.
    pub state: CiReducedState,
    /// Deterministically ordered selected jobs.
    pub jobs: Vec<CiReducedJob>,
    /// Number of selected jobs in terminal states.
    pub jobs_terminal: usize,
    /// Total number of selected jobs.
    pub jobs_total: usize,
    /// Required job identifiers whose terminal result is unsuccessful.
    pub required_failing: Vec<String>,
    /// Fail-closed explanation when infrastructure prevents trusted reduction.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum CiReducerError {
    /// The caller's expected source object differs from the validated request.
    #[error("expected SHA {requested} does not match resolved SHA {resolved}")]
    ShaMismatch {
        /// Source object identifier required by the caller.
        requested: String,
        /// Source object identifier bound into the validated request.
        resolved: String,
    },
}
