...
use buzz_core::{
    ci::{
        artifact_reference_tags, evidence_finalized_tags, job_status_tags,
        log_reference_tags, request_tags, run_status_tags, teardown_attestation_tags,
        validate_signed_ci_event, CiArtifactReferenceEnvelope, CiEvidenceFinalizedEnvelope,
        ...
        CiTeardownAttestationEnvelope, CiTeardownLease, ValidatedCiEnvelope,
        CI_SCHEMA_VERSION,
    },
    kind::{
...
fn valid_log_reference() -> CiLogReferenceEnvelope {
    CiLogReferenceEnvelope {
        ...
        job_id: "rust_lint".into(),
        attempt: 1,
        log_sha256: "a948904f2f0f479b8f8197694b30184b0d2ed1c1cd2a1ec0fb85d299a192a447"
            .into(),
        byte_length: 12,
        cap_bytes: 1024,
        truncated: false,
...
fn all_status_kinds_reject_an_empty_or_unrelated_authority_set() {
    ...
    for kind in STATUS_KINDS {
        ...
        let err = validate_signed_ci_event(&event, CHANNEL, &set_of(&other.public_key().to_hex()))
            .expect_err(&format!("kind {kind} must reject an unrelated authority set"));
        assert!(
            err.0.contains("unauthorized CI status signer"),
            "kind {kind}: expected signer-authorization error, got {err:?}"
        );
...
fn grant_set_gate_is_the_same_fail_closed_rule_as_the_static_gate() {
    ...
    validate_signed_ci_event(&event, CHANNEL, &grant)
        .expect("granted signer event passes");
