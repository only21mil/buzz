...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let auth_header = crate::api::relay_members::unique_header(&parts.headers, "authorization")
            .ok_or_else(|| {
                Response::builder()
        ...
        let is_rest_read =
            parts.uri.path().ends_with("/snapshot") || parts.uri.path().ends_with("/branches");
...
mod track_c_tests {
    ...
    fn git_branches_expected_url_is_exact_rest_url() {
        ...
        let expected = git_expected_url(
            "wss://config-host.example",
            &tenant,
            "/git/owner/repo/branches?offset=200&limit=200",
        )
        .expect("recognized branches path");

        assert_eq!(
            expected,
            "https://host-a.example/git/owner/repo/branches?offset=200&limit=200"
        );
    }
