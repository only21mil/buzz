...
//! - **Policy bounds** — see the `policy_unavailable` rejection below. The
//!   relay process cannot reach the root-owned broker policy store at
//!   request time (no in-process seam), so the handler fails closed with a
//!   precise 503 rather than inventing defaults. The broker policy
//!   (`RootOwnedPolicy`/`MaterializationLimits`) is distinct from the
//!   relay config's status-signer set (`config.ci_status_signer_pubkeys`).
...
pub async fn ci_preflight(
    ...
    // Policy bounds — no producer in this process or this SHA (see module
    // docs). `resolve_policy` fails closed with a precise 503 instead of
    // inventing defaults. Every other field above resolves normally; the
    // response is completed as soon as a policy producer is wired.
    let policy = resolve_policy(&request.target_repo_a)?;
...
/// Resolve the broker policy bounds for a preflight.
///
/// There is **no producer** for `PreflightPolicy` in the relay process at this
/// SHA. The bounds the response needs (`min_timeout_seconds`,
/// `max_timeout_seconds`, `max_expiry_seconds`,
/// `acknowledgement_timeout_seconds`, `max_attempts`) are broker-side
/// execution bounds: they come from the root-owned broker/execd policy
/// (`RootOwnedPolicy` + `MaterializationLimits`, `crates/buzz-ci-materializer/
/// src/plan.rs` and `src/manifest.rs`), which is constructed and enforced
/// outside the relay process. The relay configuration is a different
/// authority: `config.ci_status_signer_pubkeys` (config.rs:213/735) is the
/// signer-set that authorizes who may *publish* kinds 46101–46106, not a
/// producer of these execution bounds. The relay cannot reach the broker
/// policy store at request time, so it fails closed with a precise 503.
/// Every other response field has resolved by this point; wiring a
/// relay→broker policy client later completes the response without changing
/// the error class.
fn resolve_policy(target_repo_a: &str) -> Result<PreflightPolicy, (StatusCode, Json<Value>)> {
    Err(policy_unavailable(target_repo_a))
}

/// Fail-closed policy rejection: the preflight policy bounds
/// (`min_timeout_seconds`, `max_timeout_seconds`, `max_expiry_seconds`,
/// `acknowledgement_timeout_seconds`, `max_attempts`) have no producer in the
/// relay process at this SHA.
///
/// The broker/root-owned policy object exists only broker-side
/// (`RootOwnedPolicy::new` + `MaterializationLimits`, `crates/buzz-ci-
/// materializer/src/plan.rs` and `src/manifest.rs`; execd/broker-side, never
/// materialized into the relay), and the relay configuration plumbs only the
/// status-signer set (`config.ci_status_signer_pubkeys` at config.rs:213),
/// which authorizes who may sign status/evidence events — a different
/// authority, never these execution bounds. There is no relay→broker policy
/// client wired at this SHA, so the relay process cannot reach the broker
/// policy store at request time. Returning the complete response with
/// invented bounds would couple the CLI's signed request (timeout/expiry) to
/// non-authoritative numbers; failing closed is the only correct behavior.
/// Precisely cited as a 503 so a later policy client can be added without
/// changing the error class.
fn policy_unavailable(target_repo_a: &str) -> (StatusCode, Json<Value>) {
    let policy_seam = concat!(
        "broker-owned policy store: no in-process producer at a7d497ffc ",
        "(crates/buzz-ci-materializer/src/plan.rs RootOwnedPolicy + src/manifest.rs MaterializationLimits are broker/execd-side only; ",
        "relay config.ci_status_signer_pubkeys (config.rs:213) is the status-signer set, not these bounds)",
    );
    api_error(
        StatusCode::SERVICE_UNAVAILABLE,
        &format!(
            "CI preflight policy bounds unavailable for {target_repo_a}: \
             no authoritative broker policy is reachable from the relay process at this SHA \
             ({policy_seam})"
        ),
    )
}
    ...
    for stored in &roots {
        ...
        if effective_tip == tip_oid {
            ...
            effective.push(EffectivePrSnapshot {
                ...
                source_clone_url: effective_clone_url(
                    latest_update.map(|update| update).or(Some(root)),
                )
                .ok_or_else(|| {
                    api_error(
                        StatusCode::NOT_FOUND,
...
fn event_tag<'a>(event: &'a nostr::Event, name: &str) -> Option<String> {
    event
        .tags
        .iter()
...
) -> Result<(Vec<PreflightJob>, Vec<String>), (StatusCode, Json<Value>)> {
    if parsed.is_empty() {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
...
mod tests {
    ...
    async fn route_unknown_repo_is_404_not_501_not_500() {
        ...
        let response = preflight_request(harness.state.clone(), &harness.host, &harness.owner, body)
            .await;
        assert!(
...
"#,
    ...
    /// The policy seam produces the precise 503 the contract requires rather
    /// than invented bounds.
    #[test]
    fn policy_resolution_fails_closed_with_503() {
        let (status, body) = policy_unavailable(
        ...
        assert!(
            value["error"]
                .as_str()
                .unwrap()
                .contains("broker-owned policy store"),
            "policy error must cite the exact seam: {}",
            value["error"]
        );
    }

    // ── B1 rev-2 POST-C1R full-preflight acceptance ────────────────────────
    //
    // These tests target the C1R full-resolution contract that the sibling
    // implements in ITS OWN worktree (crates/buzz-relay/src/api/ci.rs
    // becomes a real resolver, not the 501 stub). The pure tests below run
    // on the CURRENT frozen tree because they only exercise the frozen
    // response shapes + base64/sha256. The `#[ignore]`d route tests compile
    // now, need a scratch Postgres, and are expected to pass only after C1R
    // lands (POST-C1R marker on each).
    ...
    /// Response shapes are `Serialize`-only on the stub; this shadow mirror
    ...
    fn rev2_success_response() -> serde_json::Value {
        ...
        let canonical_workflow_base64 = base64::engine::general_purpose::STANDARD
            .encode(REV2_CANONICAL_WORKFLOW);
        serde_json::json!({
            "target_repo_a": well_formed_repo_a(),
    ...
    fn preflight_full_response_every_field_present_and_typed() {
        ...
        assert_eq!(response.pr_update_event_id.as_deref(), Some("c".repeat(64).as_str()));
        assert_eq!(response.trigger_event_id.len(), 64);
        assert_eq!(response.source_clone_url, "https://git.example.invalid/buzz.git");
        assert!(!response.immutable_source_ref.is_empty());
        assert_eq!(response.immutable_source_ref, format!("refs/nostr/{}", "b".repeat(64)));

        // Tip/base/branch fields.
        assert_eq!(response.tip_oid, "a".repeat(40));
        assert_eq!(response.tip_oid.len(), 40, "tip OID must be a git object id width");
        assert_eq!(response.base_oid, "d".repeat(40));
        assert_eq!(response.base_oid.len(), response.tip_oid.len(), "base OID must share tip width");
        ...
        assert!(!response.workflow_path.starts_with('/'), "workflow path must be relative");
        assert_eq!(response.workflow_digest.len(), 64, "workflow digest must be a sha256 hex");

        // Jobs/needs/skip_policy consistency with the parsed canonical workflow.
        assert!(!response.jobs.is_empty(), "workflow job set must not be empty");
        ...
        assert!(!response.selected_job_ids.is_empty(), "selected jobs must not be empty");
        let selected_unique: std::collections::HashSet<&str> =
            response.selected_job_ids.iter().map(String::as_str).collect();
        assert_eq!(selected_unique.len(), response.selected_job_ids.len(), "selected job ids must be unique");
    ...
    fn preflight_trigger_event_follows_effective_pr_event() {
        ...
        no_update.as_object_mut().unwrap().remove("pr_update_event_id");
    ...
    fn preflight_policy_bounds_are_from_authoritative_source_not_defaults() {
        let response: PreflightPolicyShadow = serde_json::from_value(
            rev2_success_response()["policy"].clone(),
        )
    ...
    // ── POST-C1R route-bearing full-preflight acceptance (needs scratch DB) ──

    /// POST-C1R + scratch DB: the full route must return a deterministic
    /// resolution-domain outcome, never the dead B1 501 or a blanket
    /// "workflow not wired" 400. The controller (2026-08-26) ruled that no
    /// shared broker-enforced policy producer exists at this SHA, so the
    /// handler must not claim a 200 full response: this test asserts the
    /// member's request, for a resolvable repo (seeded 30617), lands in a real
    /// resolution outcome. The repo resolves (repo+membership pass, exact 403
    /// proven by `route_non_member_is_exactly_403`); after authorization the
    /// handle proceeds to git manifest + workflow resolution, which the
    /// DB-only route harness cannot provide (git CAS store absent), so the
    /// deterministic fail-closed is the "repository/store unavailable" class.
    /// The full preflight → policy-unavailable 503 is preserved and proven at
    /// the seam unit test `policy_resolution_fails_closed_with_503` (no shared
    /// broker producer at a7d497ffc — the B2 policy slice supplies it before a
    /// 200/503-policy route test can pass).
    #[tokio::test]
