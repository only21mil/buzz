...
use super::channel_authz::{self, ChannelAuthzError, RemoveOtherDecision};
...
async fn store_group_members_event(
    ...
    let agent_pubkeys: HashSet<Vec<u8>> = state
        ...
        .into_iter()
        .collect();
    let tags = group_member_tags(&group_id, &member_snapshot.members, &agent_pubkeys)?;
...
pub(crate) async fn handle_git_repo_announcement_inner(
    ...
    let provisioning_operation = async {
    ...
    let provisioning_result = match serving_write.protect(provisioning_operation).await {
        Ok(result) => result,
        Err(error) => Err(error),
    };
