...
pub async fn filter_fanout_by_access(
    ...
    // Historical engrams must still bind the envelope owner to the current
    // agent owner before any local, Redis or mesh delivery.
    if event_kind_u32(&stored_event.event) == buzz_core::kind::KIND_AGENT_ENGRAM
        && !super::req::engram_owner_bound(&state.db, community_id, &stored_event.event).await
    {
        return Vec::new();
    }
...
mod tests {
    ...
    mod pubsub_fanout_postgres_tests {
        ...
        async fn presence_redis_failure_rejects_and_skips_fanout() {
            ...
            assert!(
                matches!(error, crate::handlers::ingest::IngestError::Internal(message) if message == "error: presence storage unavailable")
            );
