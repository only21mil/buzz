...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            // The event is intentionally relay-signed. Materialize that sink
            // identity as an agent owned by the workflow author so ACP's
            // owner/sibling author gate can attribute the event without
            // changing the signer or p-tag contract. Concurrent sends for the
            // same owner are idempotent; a conflicting owner fails closed.
            let relay_pubkey_bytes = state.relay_keypair.public_key().to_bytes().to_vec();
            let associated = state
                .db
                .ensure_agent_owner_association(
                    tenant.community(),
                    &relay_pubkey_bytes,
                    &author_pubkey_bytes,
                )
            ...
            if !associated {
                return Err(ActionSinkError::Database(
                    "workflow sink identity is already associated with a different owner".into(),
                ));
            }
