// P01 fork contracts: mention clipboard identity behaviors adopted from upstream.
// New file. The fork-native mention-clipboard.spec.ts keeps its condensed checks.
// This file restores the upstream regression behaviors the condensed file dropped,
// mapped one scenario per kept behavior (audit D3).
...
test.beforeEach(async ({ page }) => {
  // Fork note: upstream resolves "John Smith" through a mockDisplayNames seed
  // (MULTI_WORD_NON_MEMBER_PUBKEY) that the fork bridge dropped. Seeding the
  // same profile per test restores identical verification semantics without
  // touching app source.
...
async function emitMentionMessage(page: Page, channelName: string) {
  ...
  // Fork note: the fork timeline marks bodies with .message-markdown rather
  // than the upstream message-body testid. Scope to the emitted row instead.
  const chip = page
    .locator(`[data-message-id="${event.id}"] .message-markdown`)
...
// QUARANTINE — needs the __BUZZ_E2E_LAST_CLIPBOARD__ capture seam, which the
// fork bridge does not implement (upstream e2eBridge only). Hand to P06 with
// the mockDisplayNames seed gap.
test.fixme("copy message writes the identity sidecar beside readable plain text", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-general").click();
...
// fork bridge does not implement (upstream e2eBridge only). Hand to P06 with
// the mockDisplayNames seed gap.
test.fixme("home inbox copy message carries the mention out of the detail pane", async ({
  page,
}) => {
  await page.goto("/");
...
// QUARANTINE — needs the __BUZZ_E2E_HOLD_USERS_BATCH__ relay-lookup hold seam,
// which the fork bridge does not implement (upstream e2eBridge only). Without the
// seam the paste verifies from local state instantly and the race has no setup.
// Hand to the P06 desktop owner with the mockDisplayNames seed gap below.
test.fixme("sending on top of a paste still being verified keeps its identity", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-bob-tyler").click();
...
// seam the paste verifies from local state instantly and the race has no setup.
// Hand to the P06 desktop owner with the mockDisplayNames seed gap below.
test.fixme("a slow paste settling last does not take a newer paste's name", async ({
  page,
}) => {
  ...
  await installMockBridge(page, {
    searchProfiles: [
      { pubkey: NAMESAKE_PUBKEY, displayName: MENTION_LABEL },
      { pubkey: JOHN_SMITH_PUBKEY, displayName: MENTION_LABEL },
    ],
  });
  await page.goto("/");
  await page.getByTestId("channel-general").click();
...
// seam the paste verifies from local state instantly and the race has no setup.
// Hand to the P06 desktop owner with the mockDisplayNames seed gap below.
test.fixme("retyping the pasted mention binds nothing to the typed words", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-bob-tyler").click();
...
// seam the paste verifies from local state instantly and the race has no setup.
// Hand to the P06 desktop owner with the mockDisplayNames seed gap below.
test.fixme("editing a word beside a pasted mention keeps its identity", async ({
  page,
}) => {
...
// seam the paste verifies from local state instantly and the race has no setup.
// Hand to the P06 desktop owner with the mockDisplayNames seed gap below.
test.fixme("a deleted paste binds nothing to the same name typed after it", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-bob-tyler").click();
