// P01 fork contracts: persistent agent audience behaviors adopted from upstream.
// New file. The existing persistent-agent-audience.spec.ts keeps the fork-native
// condensed coverage. This file restores the upstream regression behaviors that
// the condensed file dropped, mapped one test per kept behavior (audit D3).
...
test("keeps the composer automatic mention setting synchronized with the picker", async ({
  page,
}) => {
  // Fork note: upstream also asserts a global Settings toggle
  // (settings-automatic-agent-mentions), but the fork settings agents panel
  // has no such toggle. The composer-side sync below is the fork-supported
  // contract. The missing global toggle is mapped in p01-evidence-map.md.
  await installAudienceFixtures(page);
  await openThread(page);
...
// QUARANTINE — fork divergence, reproduced 2026-09-13 in this file: Shift+M in
// the thread composer addresses the default agent (alice) even with a Vogue
// mention visible in the timeline. Upstream passes recentMentionPubkeys to both
// composers (ChannelPane.tsx). The fork passes it only to the main composer
// (ChannelPane.tsx:707). MessageThreadPanel.tsx mounts the thread composer
// without the prop. Hand to the P06 desktop owner: either wire the prop
// through or record an intentional exclusion, then un-quarantine.
test.fixme("primary+Shift+M favors the most recently mentioned eligible agent", async ({
  page,
}) => {
  await installAudienceFixtures(page);
  await openGeneral(page);
  ...
  await expect(
    page
      ...
      .getByText("Please ask Vogue", { exact: false }),
  ).toBeVisible();
  await openThread(page);
