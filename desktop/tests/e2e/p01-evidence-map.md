...
| `p01-mention-clipboard-contracts.spec.ts` (5 active, 10 quarantined) | Timeline/channel/forum copies, composer round-trip, half-chip text, NBSP paste (active). Paste-verify races, retype/edit/delete binding (quarantined, need the missing relay-lookup hold seam). Sidecar and inbox copies (quarantined, need the missing clipboard-capture seam). Bare-text pastes naming a known non-member (quarantined, the fork-only send gate in `unresolvedMentionFeedback.ts` rejects what upstream binds and sends) | `mention-clipboard.spec.ts`. The fork bridge dropped the upstream `mockDisplayNames` John Smith seed, so the file seeds it per test. |
