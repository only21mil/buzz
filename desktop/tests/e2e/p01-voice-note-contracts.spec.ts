// P01 fork contracts: voice note composer behaviors adopted from upstream.
// New file. The fork-native voice-note.spec.ts keeps its condensed checks.
...
// All tests run service-free: the mic is faked and uploads resolve from fixtures.
...
// QUARANTINE — reproduced 2026-09-13 in this file: the "Loading voice note"
// pending state never appears because gating AUDIO_URL with page.route does not
// hold the load. The fork resolves attachment audio through the media proxy
// (shared/lib/mediaUrl.ts), not a direct fetch of the imeta URL, so the gate
// never engages. Hand to P06: gate the proxy fetch or expose a load-hold seam,
// then un-quarantine.
test.fixme("resumes a Play click that landed before the source finished loading", async ({
  page,
}) => {
...
// QUARANTINE — needs the media-fetch hold/state seams (__BUZZ_E2E_HOLD_MEDIA_FETCHES__,
// __BUZZ_E2E_MEDIA_FETCH_STATE__) that the fork bridge does not implement. The fork
// loads attachment audio through the media proxy, so the held-fetch setup cannot
// stage the queued-load window either. Hand to P06 with the proxy path note.
test.fixme("hard-caps audio work and cancels active and queued loads on unmount", async ({
  page,
}) => {
  await page.addInitScript(() => {
...
test("keeps emoji available beside a queued voice note", async ({ page }) => {
  // Fork note: upstream asserts a GIF tab that starts disabled beside voice
  // notes. The fork picker is emoji-only (ComposerEmojiPicker embeds a single
  // EmojiPicker, no tabs), so the contract pins what the fork has: emoji stays
  // available next to the queued note and no GIF surface exists.
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await page.getByRole("button", { name: "Record voice note" }).click();
  await page.waitForTimeout(100);
