...
// Ports from block/buzz@4cd82f51 desktop/tests/e2e/persistent-agent-audience.spec.ts
// (audit D3), verbatim except for two renames: installAutoMentionFixtures and
// openBareThread collide with the fork helpers above, which additionally pin the
// session audience through the UI — the ports need the plain navigation/install.
// Shared consts (SHOTS, CHANNEL_ID, AGENT_A/B, THREAD_ROOT_ID) and the identical
// openGeneral/channelComposer/threadComposer helpers are reused, not duplicated.
// Not ported: the composer/global settings sync test (the fork has no
// settings-page automatic-mentions switch) and the most-recent-agent recency test
// (the fork shortcut addresses the default eligible agent, not the most recent).
...
async function openBareThread(page: Page, threadRootId = THREAD_ROOT_ID) {
  await page.goto(
    `/#/channels/${CHANNEL_ID}?messageId=${threadRootId}&thread=${threadRootId}`,
    { waitUntil: "domcontentloaded" },
  );
...
async function installAutoMentionFixtures(
  page: Page,
  options: {
    agentAName?: string;
...
test("keeps a queued-attachment send locked through upload and send settlement", async ({
  ...
  await installAutoMentionFixtures(page, {
...
test("the unfocused root composer keeps its dismissed mention menu closed through a thread send", async ({
  ...
  await installAutoMentionFixtures(page, { sendMessageDelayMs: 1_500 });
  await openGeneral(page);
...
test("automatic mentions exist only in thread composers and stay thread-scoped", async ({
  ...
  await openBareThread(page, "mock-general-alice");
  await expect(
    threadComposer(page).getByTestId(`composer-address-lock-${AGENT_B}`),
  ).toHaveCount(0);
});
...
test("a root agent mention is explicit for one message and never becomes retained", async ({
  ...
  await keepMentionedAgentsPinned(page);
  await installAutoMentionFixtures(page, { agentAName: "claude code" });
  await openGeneral(page);
