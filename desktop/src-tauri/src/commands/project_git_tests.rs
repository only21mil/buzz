...
fn snapshot_reports_exact_commit_count_beyond_preview_limit() {
    let auth = super::super::project_git_exec::build_test_git_auth_config()
        .expect("build test git config");
    ...
    for snapshot in [
        snapshot_from_worktree(repo, &auth, Some("main"), Some("main")),
        snapshot_from_repo(repo, &auth, Some("main"), Some("main")),
    ] {
