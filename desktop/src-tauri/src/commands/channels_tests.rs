...
const RELAY_A: &str = "https://relay-a.example";
const RELAY_B: &str = "https://relay-b.example";
...
fn cached_agent_profile_survives_timeout_and_empty_lookup() {
    ...
    let membership = ev(
        39002,
        "",
        vec![vec!["d", "chan-1"], vec!["p", &agent_pubkey, "", "admin"]],
    );
    let mut profile_cache = std::collections::HashMap::new();
    ...
    let cache_key = (RELAY_A.to_string(), agent_pubkey.clone());
    let cached = profile_cache.get(&cache_key).unwrap();
    assert!(cached.is_agent);
    assert_eq!(cached.display_name.as_deref(), Some("Admin agent"));
...
fn unparseable_owned_profile_still_marks_and_caches_agent() {
    ...
    let cached = profile_cache
        .get(&(RELAY_A.to_string(), agent_pubkey))
        .unwrap();
    assert!(cached.is_agent);
    assert_eq!(cached.display_name, None);
}
...
fn valid_non_agent_profile_clears_cached_agent_status() {
    ...
    let agent_pubkey = agent_profile.pubkey.to_hex();
    let human_profile = EventBuilder::new(Kind::Metadata, r#"{"display_name":"Human profile"}"#)
        .sign_with_keys(&agent_keys)
        .unwrap();
    let membership = ev(
        39002,
...
#[test]
fn verified_agent_cache_is_scoped_to_relay() {
    let agent_profile = oa_profile_event(r#"{"display_name":"Admin agent"}"#);
    let agent_pubkey = agent_profile.pubkey.to_hex();
    let membership = ev(
        39002,
    ...
    let mut profile_cache = std::collections::HashMap::new();
    let mut relay_a = nostr_convert::channel_members_from_event(&membership).unwrap();
    enrich_channel_members_from_profile_events(
        &mut relay_a,
        Ok::<_, &str>(std::slice::from_ref(&agent_profile)),
        RELAY_A,
    ...
    assert!(relay_a.members[0].is_agent);

    let mut relay_b = nostr_convert::channel_members_from_event(&membership).unwrap();
    enrich_channel_members_from_profile_events(
        &mut relay_b,
        Ok::<_, &str>(&[]),
        RELAY_B,
    ...
    assert!(!relay_b.members[0].is_agent);
}

#[test]
fn late_older_positive_cannot_restore_newer_revocation() {
    let agent_keys = Keys::generate();
    let agent_profile =
        oa_profile_event_with_keys(r#"{"display_name":"Former agent"}"#, &agent_keys);
    let agent_pubkey = agent_profile.pubkey.to_hex();
    ...
    let mut newer = nostr_convert::channel_members_from_event(&membership).unwrap();
    enrich_channel_members_from_profile_events(
        &mut newer,
        Ok::<_, &str>(std::slice::from_ref(&human_profile)),
        RELAY_A,
    ...
    assert!(!newer.members[0].is_agent);

    let mut late_older = nostr_convert::channel_members_from_event(&membership).unwrap();
    enrich_channel_members_from_profile_events(
        &mut late_older,
        Ok::<_, &str>(std::slice::from_ref(&agent_profile)),
        RELAY_A,
    ...
    assert!(!late_older.members[0].is_agent);
    let cached = profile_cache
        .get(&(RELAY_A.to_string(), agent_pubkey))
        .unwrap();
    assert!(!cached.is_agent);
