...
pub(crate) use runtime_metadata::{EffortNormalization, KnownAcpRuntime};
...
///
/// Older 1.x adapters are detected successfully, but can bundle a Codex runtime
/// that rejects newer models: 1.6.2 has Codex 0.148.x (no Astra), 1.10.0 has `^0.153.3`.
///
    ...
    PartialEntry {
        ...
        entry: AcpRuntimeCatalogEntry {
            ...
            effort_canonical_values: runtime
                ...
                .map(|n| n.canonical.iter().map(|s| s.to_string()).collect()),
