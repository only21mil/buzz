...
mod process_tests {
    ...
    #[test]
    fn selected_policy_reaches_real_child_and_provider_environment() {
        for policy in [AcpSessionPolicy::Channel, AcpSessionPolicy::Thread] {
            let mut command = std::process::Command::new("/bin/sh");
            command
                .env_clear()
                .env(ACP_SESSION_POLICY_ENV_VAR, "descriptor");
            command.args(["-c", "printf '%s' \"$BUZZ_ACP_SESSION_POLICY\""]);
            apply_acp_session_policy_env(&mut command, policy);
            let output = command.output().unwrap();
            assert!(output.status.success());
            assert_eq!(String::from_utf8(output.stdout).unwrap(), policy.as_str());
            let mut provider_env = BTreeMap::new();
            insert_acp_session_policy_env(&mut provider_env, policy);
            assert_eq!(provider_env[ACP_SESSION_POLICY_ENV_VAR], policy.as_str());
        }
    }
}
