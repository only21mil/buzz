...
test("storage clear resets mounted consumers and ignores unrelated keys", () => {
  preference.setVideoPlaybackSpeed(2);
  let notifications = 0;
  const unsubscribe = preference.subscribeToVideoPlaybackSpeed(
    () => notifications++,
  ...
  values.clear();
  windowListeners.get("storage")({ key: "unrelated" });
  assert.equal(preference.getVideoPlaybackSpeed(), 2);
  assert.equal(notifications, 0);
  windowListeners.get("storage")({ key: null });
  assert.equal(preference.getVideoPlaybackSpeed(), 1);
  assert.equal(notifications, 1);
  unsubscribe();
});

test("unavailable storage leaves the in-memory preference usable", () => {
  ...
  try {
    preference.setVideoPlaybackSpeed(1.75);
    assert.equal(preference.getVideoPlaybackSpeed(), 1.75);
    windowListeners.get("storage")({ key: null });
    assert.equal(preference.getVideoPlaybackSpeed(), 1);
    for (const speed of [NaN, Infinity, -1, 0])
      preference.setVideoPlaybackSpeed(speed);
    assert.equal(preference.getVideoPlaybackSpeed(), 1);
  } finally {
