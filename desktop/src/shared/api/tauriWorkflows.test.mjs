...
const { parseWorkflowRunsPage, parseWorkflowApprovals } = await import("./tauriWorkflows.ts");
const run = {
  id: "run", workflow_id: "workflow", status: "resume_pending", current_step: 2,
  execution_trace: [{step_id: "gate", status: "waiting_approval"}],
  started_at: 1, completed_at: null, error_message: null, created_at: 1,
};
...
test("paged history preserves cursor precision and structured error", () => {
  ...
  const page = parseWorkflowRunsPage({runs: [{...run, status: "failed", error_code: "step_timeout"}], next});
...
test("malformed history envelope and status fail instead of becoming empty history", () => {
  for (const value of [null, {}, {runs: []}, {runs: [], next: {}}, [{...run, status: "invented"}]]) {
...
test("durable and legacy evidence is display-only and rejects decision-token shapes", () => {
  const approval = {approval_ref: "public-reference", workflow_id: "workflow", run_id: "run", step_id: "gate", step_index: 2, approver_spec: "owner", status: "granted", approver_pubkey: null, note: null, expires_at: "2026-09-06T00:00:00Z", created_at: 1};
  assert.equal(parseWorkflowApprovals({approvals: [approval]})[0].approvalRef, "public-reference");
  assert.equal(parseWorkflowApprovals({approvals: [approval]})[0].status, "granted");
  assert.equal("token" in parseWorkflowApprovals([approval])[0], false);
  assert.throws(() => parseWorkflowApprovals([{...approval, token: "hash"}]), /display reference/);
  assert.throws(() => parseWorkflowApprovals([{...approval, approval_ref: undefined}]), /display reference/);
});
test("duplicate accepted trigger preserves original event and null run", () => {
  assert.deepEqual(parseRawTriggerWorkflowResponse({event_id: "original", workflow_id: "workflow", run_id: null, status: "accepted"}), {event_id: "original", workflow_id: "workflow", run_id: null, status: "accepted"});
});
