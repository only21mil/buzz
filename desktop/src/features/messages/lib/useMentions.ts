...
import { useCandidateProfiles } from "./useMentionCandidateProfiles";
...
import { appendUniqueName } from "./mentionUserLabels";
import {
  ...
  const agentIdentityPubkeys = React.useMemo(
    () =>
      getAgentIdentityPubkeys({
        ...
        relayAgents: relayAgentsQuery.data ?? [],
        members: members ?? [],
        profileIsAgent: (pubkey) => candidateProfiles[pubkey]?.isAgent === true,
      }),
    [managedAgentPubkeys, relayAgentsQuery.data, members, candidateProfiles],
  );
