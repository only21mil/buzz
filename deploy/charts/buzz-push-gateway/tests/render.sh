...
production_args=(
  ...
  --set 'profiles.dogfood.appAttestAppId=REALTEAM.com.buzz.buzzMobile'
...
# App Attest and APNs must describe the same server-owned application.
if helm template push deploy/charts/buzz-push-gateway \
  --set profiles.dogfood.appAttestAppId=TEAM.other.bundle >/dev/null 2>&1; then
  echo 'mismatched App Attest identity and APNs topic must fail rendering' >&2
  exit 1
fi
