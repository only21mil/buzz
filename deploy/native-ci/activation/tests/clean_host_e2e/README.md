...
3. The candidate boot uses its own overlay over the frozen ceremony image. It
   ...
   only the fixed `BSP\x01` header and a contiguous sequence of 46 ordinal/
   complement pairs. A failed or timed-out stage may authenticate only the last
   valid fixed subphase; malformed or missing bytes remain the generic
   `controller_stage` phase. A successful controller exit requires the complete
   96-byte stream, and these diagnostic records cannot satisfy terminal
   progress completion.
