...
def validate_closed_verdict(value: object) -> dict[str, object]:
    ...
    if (
        ...
        or not isinstance(receipt.get("export_generation"), int)
        or isinstance(receipt["export_generation"], bool) or receipt["export_generation"] < 1
...
def validate_acceptance_template(value: object, *, label: str) -> dict[str, object]:
    ...
    fields = {
        ...
        "export_authorization_digest", *TEMPLATE_NAMES,
    }
...
class RelayState:
    ...
    def __init__(
        ...
        self.evidence_reads: list[dict[str, object]] = []
        self.ci_cursor = 0
    ...
    def record_evidence_read(
        ...
        self.evidence_reads.append(record)
        body = canonical_json({
            "schema_version": EVIDENCE_READS_SCHEMA,
            "export_generation": self.export_generation,
            "reads": self.evidence_reads,
        }) + b"\n"
        root = self.object_root.parent
        pending = root / (EVIDENCE_READS_RECORD_NAME + ".next")
...
def authenticate_once(
    ...
    with state.lock:
        ...
        if identifier in state.seen_tokens:
...
def store_evidence_object(
    ...
    try:
        if not _atomic_create_at(directory, digest, raw):
            try:
        ...
        if not _atomic_create_at(directory, owner_name, owner_raw):
            try:
                existing_owner = _read_regular_at(directory, owner_name, MAX_BODY)
            except OSError as error:
...
def read_evidence_object(
    ...
    try:
        ...
        if _read_regular_at(directory, owner_name, MAX_BODY) != owner_raw:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            if not parsed.query and not parsed.fragment and evidence_route_arity(parsed.path):
    ...
    def do_PUT(self) -> None:
        ...
        raw = self._body()
        try:
            if raw is None or parsed.query or parsed.fragment:
                raise Refusal(404, "not found")
            evidence_path = parse_evidence_path(parsed.path)
...
def state_from_config(config: object, object_root: Path) -> RelayState:
    if (
        ...
        or not isinstance(config["export_generation"], int)
        or isinstance(config["export_generation"], bool) or config["export_generation"] < 1
    ):
