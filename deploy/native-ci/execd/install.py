#!/usr/bin/env python3
"""Check, plan, install, or roll back a frozen Buzz CI execd package."""
...
RECEIPT_SCHEMA = "buzz-ci-execd-install-receipt-v1"
DIGEST = re.compile(r"^[0-9a-f]{64}$")
GIT_OID = re.compile(r"^[0-9a-f]{40}$")
PACKAGE_ID = re.compile(r"^buzz-ci-execd-[0-9a-f]{12}-[0-9a-f]{12}$")
DEFAULT_BACKUP_ROOT = Path("/var/lib/buzzci/install-backups/execd")
...
CONTROL_PEER_ACCOUNT = "buzzci-ctl"
CONTROL_PEER_UID = 961
CONTROL_PEER_HOME = "/var/lib/buzzci/principals/ctl"
RUNNER_PEER_ACCOUNT = "buzzci-runner"
SERVICE_SHELL = "/usr/sbin/nologin"
EXPECTED_TARGETS = {
    "binary": "/usr/libexec/buzz-ci-execd",
    "service": "/etc/systemd/system/buzz-ci-execd.service",
    "socket": "/etc/systemd/system/buzz-ci-execd.socket",
    "tmpfiles": "/usr/lib/tmpfiles.d/buzzci-execd.conf",
    "documentation": "/usr/share/doc/buzz-ci-execd/README.md",
}
EXPECTED_DIRECTORIES = {"/usr/share/doc/buzz-ci-execd"}
...
def parse_manifest(package: Path, root: Path) -> tuple[dict[str, object], list[Entry]]:
    ...
    expected_keys = {
        ...
        "default_state", "daemon_contract", "package_uid", "package_gid", "directories", "entries", "package_digest",
    }
    ...
    if manifest["default_state"] != DEFAULT_STATE or manifest["daemon_contract"] != DAEMON_CONTRACT or manifest["package_uid"] != 0 or manifest["package_gid"] != 0:
    ...
    for raw_entry in raw_entries:
        ...
        if entry.uid != 0 or entry.gid != 0 or entry.install_mode not in {0o644, 0o755}:
        ...
        if role == "binary" and entry.sha256 != provenance["sha256"]:
            raise ValueError("execd binary is not bound to provenance")
...
def validate_assets(package: Path, entries: list[Entry]) -> None:
    ...
    required_service = {
        ...
        "Requires=buzz-ci-execd.socket systemd-tmpfiles-setup.service",
        "Restart=no",
    }
    ...
    if not all(line in service_lines for line in required_service) or "[Install]" in service_lines:
        raise ValueError("execd service dormant contract mismatch")
    required_socket = {
        "ListenStream=/run/buzzci/execd.sock",
        "FileDescriptorName=buzz-ci-execd",
        "Accept=no",
        "SocketUser=root",
        "SocketGroup=root",
        "SocketMode=0666",
        "RemoveOnStop=yes",
        "Service=buzz-ci-execd.service",
    }
    if not all(line in socket.splitlines() for line in required_socket) or "[Install]" not in socket.splitlines():
        raise ValueError("execd socket contract mismatch")
    if [line for line in tmpfiles.splitlines() if line and not line.startswith("#")] != ["d /run/buzzci 0711 root root -"]:
        raise ValueError("execd tmpfiles contract mismatch")
    forbidden = ("keyholder", "relay", "runner-control", "controld")
    if any(token.lower() in (service + socket + tmpfiles).lower() for token in forbidden):
        raise ValueError("execd package crosses a forbidden subsystem boundary")
...
def validate_host_identities(root: Path, manifest: dict[str, object]) -> None:
    ...
    for account in (CONTROL_PEER_ACCOUNT, RUNNER_PEER_ACCOUNT):
        matching = [row for row in users if row[0] == account]
        if len(matching) != 1:
            raise ValueError(f"host {account} peer identity is missing or duplicated")
        fields = matching[0]
        try:
            peers[account] = (int(fields[2]), int(fields[3]))
        except ValueError as error:
            raise ValueError(f"host {account} peer identity is malformed") from error
        if fields[6] != SERVICE_SHELL:
            raise ValueError(f"host {account} login posture is invalid")
    if peers[CONTROL_PEER_ACCOUNT] != (CONTROL_PEER_UID, CONTROL_PEER_UID):
        raise ValueError(f"host {CONTROL_PEER_ACCOUNT} identity does not match the package")
    if [row for row in users if row[0] == CONTROL_PEER_ACCOUNT][0][5] != CONTROL_PEER_HOME:
        raise ValueError(f"host {CONTROL_PEER_ACCOUNT} home does not match the package")
    runner_uid, runner_gid = peers[RUNNER_PEER_ACCOUNT]
    if runner_uid == 0 or runner_gid == 0 or runner_uid == CONTROL_PEER_UID:
        raise ValueError(f"host {RUNNER_PEER_ACCOUNT} identity does not match the package")
