...
class MessageContent extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Recipients bind to the signed identity key from the event's tags,
    // never to message text alone, so two same-name people stay distinct.
    ...
    final finalContent = useMemoized(() {
      ...
      for (var i = 0; i < mentionParts.length; i++) {
        if (i.isOdd) {
          ...
          // Longest bound label wins, so a shorter or interior name can
      ...
      return result;
    }, [markdownContent, resolvedMentionNames, mentionBindings]);
