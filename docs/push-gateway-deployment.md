...
(`com.buzz.buzzMobile`). App Attest must cryptographically validate
...
When upgrading a gateway that ran before migration0005, drain and stop every old replica, then wait at least 61 seconds before admitting requests with the new version. Old binaries delete consumed issuance rows; mixing them with the new version cannot enforce its global ceiling. Fresh deployments require no legacy drain.

The service retains consumed challenges in the deployment-global 600-per-60-second issuance count until their creation time leaves the rolling window. Gateway migration0005 adds the consumed marker; readiness requires it. Consumption is atomic and never refunds quota. The service reaps consumed or expired challenges only after that issuance window, plus expired replay rows, idle quota rows, expired/revoked delegations, and retention-eligible installations (including their encrypted token ciphertext) at startup and every five minutes. Monitor reaper failures and table growth; retention does not depend on process restarts.
...
enroll `com.buzz.buzzMobile` or another bundle identifier merely by changing
...
and wait for the signed `com.buzz.buzzMobile` artifact to appear in
...
profiles for both `com.buzz.buzzMobile` and
`com.buzz.buzzMobile.NotificationService`; an app-only profile does
