...
Workflow registration is fixed: omission and ordinary name/digest selectors resolve `.github/workflows/ci.yml`; the explicit selector `native-macos` resolves `.buzz/workflows/native-macos.yml` and requires its canonical workflow name to equal `native-macos`. Each path is read only at the trusted `base_oid`. The native workflow must already exist in that base; a candidate-only file cannot register it. Missing files or selector/name mismatches return 404 `workflow_not_found`. No selector is interpreted as a filesystem path. The CLI requires the returned path to equal this registration before independently hashing the canonical bytes. Native registration leaves ordinary CI selection and protected merge-gate workflow resolution unchanged.
