...
This source contract does not activate Buzz-native CI. A production host reaches persistent capacity one only after the fixed activation controller verifies the exact canary pass receipt, confirms the canary's final capacity-zero receipt against current host state, performs `persist-capacity-one`, and independently reads back one-job capacity. Any failed or mismatched readback returns to zero or requires the controller's deterministic rollback. GitHub's protected checks remain the delivery authority until that operator sequence, the separate production adapter seams, owner-configured status signers and policy, required service credentials, review, and deployment are complete.
